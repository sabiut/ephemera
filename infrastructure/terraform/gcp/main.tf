...
# Kubernetes Namespace (must exist before secrets can be created)
resource "kubernetes_namespace" "ephemera_system" {
  metadata {
    name = "ephemera-system"
    labels = {
      name = "ephemera-system"
      app  = "ephemera"
    }
...
# NOTE: Kubernetes secrets are managed by kubectl in the GitHub Actions pipeline
# (deploy.yml) which has access to real GitHub Actions secrets.
# This avoids drift between Terraform placeholder values and actual secret values.
